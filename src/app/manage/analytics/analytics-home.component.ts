...
import { isPlatformBrowser } from '@angular/common'; //here it is used to detect browser vs server
...
export class AnalyticsHomeComponent implements OnInit {
  ...
  constructor(@Inject(PLATFORM_ID) private platformId: Object) { //detects if code is running in browser(vs server)-avoid ssr errors
    this.isBrowser = isPlatformBrowser(this.platformId);
  }

  ngOnInit(): void {
    if (this.isBrowser) {
      // Place any browser-only logic here safely (e.g., window, localStorage)
      // Safe to use browser-only features here (window, localStorage, etc.)
      // Example (future use):
