...
import { DateValidator, TimeValidator } from 'src/app/_helpers/must-match.validator';
...
export class CalendarComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.defaultDate = this.todayAsStr;
    ...
    loadCalendarOnSubmit() {
        ...
        if (this.selectedSlotListArr.length == 0) {
            this.messageService.add({ key: 'calendarSearchtoastmsg', severity: 'error', summary: 'error', detail: 'Please select at least one slot', life: 6000 });
            setTimeout(() => {
            }, 2000);
            return;
        }
    ...
    getSlotsList(event: LazyLoadEvent) {
        ...
        // if (event.filters != undefined && event.filters["sr_no"] != undefined) {
        //     params += "&filterBySrNo=" + event.filters["sr_no"].value;
        // }
        // if (event.filters != undefined && event.filters["startTime"] != undefined) {
        //     params += "&filterBystartTime=" + event.filters["startTime"].value;
        // }
        // if (event.filters != undefined && event.filters["endTime"] != undefined) {
        //     params += "&filterByendTime=" + event.filters["endTime"].value;
        // }
    ...
    */
    ...
    exportExcel() {
        ...
        //this.getSlotsList(this.lastTableLazyLoadEvent);
        let propertiesRemove = ['slotRecurringStatus', 'created_by', 'enduserId', 'enduserName', 'status', 'role', 'profilepic', 'createdby', 'updatedby', 'disable', 'countrycode', 'statecode', 'trainerId', 'courseId', 'courseCategory', 'id', 'startTime', 'endTime', 'createdBy', 'created_at', 'updatedby', 'updated_at', 'roleId'];
        this.downloadUserList = this.slotListArr;
        console.log(this.downloadUserList);
