...
export class AddComponent implements OnInit {
    ...
    onSubmit() {
        ...
        this.userService.addUser(userData).subscribe(
            data => {
                this.messageService.add({ key: 'toastmsg', severity: 'success', summary: 'Successful', detail: 'Caterer Added', life: 6000 });
                setTimeout(() => {
                    this.router.navigate(['/manage/caterer/list']);
                }, 2000);
            },
            ((err) => {
                this.messageService.add({ key: 'toastmsg', severity: 'error', summary: err.error.message, detail: 'Add Caterer failed', life: 6000 });
            })
        );
    }
