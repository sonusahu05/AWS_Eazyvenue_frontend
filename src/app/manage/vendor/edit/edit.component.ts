...
export class EditComponent implements OnInit {
    ...
    onSubmit() {
        ...
        this.userService.updateUser(this.id, userData).subscribe(
            data => {
                this.messageService.add({ key: 'toastmsg', severity: 'success', summary: 'Successful', detail: 'Vendor Updated', life: 6000 });
                setTimeout(() => {
                    this.router.navigate(['/manage/vendor/list']);
                }, 2000);
            },
            ((err) => {
                this.messageService.add({ key: 'toastmsg', severity: 'error', summary: err.error.message, detail: 'Update Vendor failed', life: 6000 });
            })
        );
    }
